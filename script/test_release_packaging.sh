...
PUBLISHED_PACKAGE="$ROOT/installer/PenguinFan-1.2.2.pkg"
FINAL_PACKAGE=""
LOCK_FILE=""
...
FINAL_PACKAGE="$TEST_INSTALLER_DIR/PenguinFan-1.2.2.pkg"
LOCK_FILE="$TEST_INSTALLER_DIR/.PenguinFan-1.2.2.publication.lock"
...
assert_no_app_staging_directories() {
  ...
  if /usr/bin/find "$output_root" -maxdepth 1 -type d \
    -name ".PenguinFan-1.2.2.app-staging.*" \
    -print -quit | /usr/bin/grep -q .; then
...
DIRECT_APP="$DIRECT_OUTPUT_ROOT/dist-1.2.2/PenguinFan.app"
...
DIRECT_APP_LOCK="$DIRECT_OUTPUT_ROOT/.PenguinFan-1.2.2.app-publication.lock"
